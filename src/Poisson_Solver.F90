...
! ALLOCATE(Amat(NCELL,NCELL))
 ALLOCATE(phi(NCELL),rhs(NCELL))
 ALLOCATE(PBC(NCELL))
...
!	PRINT*,'ITERATION', it, 'ERROR', L2Norm
 ...
 SUBROUTINE SOLVE_PRESSURE_TDMA
 ...
 DOUBLE PRECISION :: Mat(nxc,nxc), dv(nxc), res(nxc)
 ...
 DO WHILE ((.NOT. FLAG) .AND. it .LT. PITERMAX)
	...
	DO jc = 1, nyc
		...
		DO ic = 1, nxc
			...
			dv(ic) = RHS(ICELL) - AmatPG(ICELLG,icN)*pc(icG,jcG+1) &
								- AmatPG(ICELLG,icS)*pc(icG,jcG-1)

			IF (ic .NE. 1) THEN
				Mat(ic,ic-1) = AmatPG(ICELLG,icW)
			ELSE
				dv(ic) = dv(ic) - AmatPG(ICELLG,icW)*pc(icG-1,jcG)
			END IF
			Mat(ic,ic)   = AmatPG(ICELLG,ICELLG)
			IF (ic .NE. nxc) THEN
				Mat(ic,ic+1) = AmatPG(ICELLG,icE)
			ELSE
				dv(ic) = dv(ic) - AmatPG(ICELLG,icE)*pc(icG+1,jcG)
			END IF
	 	END DO

		CALL TDMA(Mat,dv,res,nxc)

		pc(2:nxcg-1,jcG) = (1.0-omega)*pc(2:nxcg-1,jcG)  + omega*res
...
!	PRINT*,'ITERATION', it, 'ERROR', L2Norm
 ...
 END SUBROUTINE SOLVE_PRESSURE_TDMA
 !----------------------------------------------------------------------------!
